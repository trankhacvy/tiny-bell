...
pub async fn set_pref<R: Runtime>(
    ...
    let updated = match key.as_str() {
        ...
        "hide_to_menubar_shown" | "start_at_login" | "show_in_dock" => {
            let b = value
                .as_bool()
    ...
    apply_side_effects(&app, &key, &updated);
...
fn apply_side_effects<R: Runtime>(app: &AppHandle<R>, key: &str, prefs: &Prefs) {
    match key {
        "refresh_interval_ms" => {
        ...
        "global_shortcut" => {
            if let Err(e) = crate::shortcut::register(app, &prefs.global_shortcut) {
                log::warn!("register shortcut failed: {e}");
            }
        }
        "start_at_login" => {
